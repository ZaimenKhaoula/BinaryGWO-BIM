...
#nb_zones=15*5+5*2
zones, obstacles, dalles = getZones(ifc_file, NumEtage, 2, ParPlafond=True)
print("nb zones  ", end="  ")
print(len(zones))
Rc = 6
Rs = 4
...
def solve(run):
    pop= create_pop()
    fichier="resultats_variation_"+str(run)
    for current_epoch in range(0, epoch):
        #nonlineair convergence factor
        a = 2 - 2 * current_epoch / (epoch - 1)
        #z = pow(((epoch - current_epoch) / epoch), 2)
        #a = 2 - 2 * (math.sqrt(1 - z))
        #weighting strategy
        ...
        for idx in range(0, pop_size):
            ...
            #pos_new = ((list_best[0].fitness/weigth)*X1 + (list_best[1].fitness/weigth)*X2 + (list_best[2].fitness/weigth)*X3) / 3.0
            # dynamic variation strategy
            pos_new = (X1 + X2 + X3) / 3.0
            mutation = False
            if random.random() < 0.5 * ((epoch - current_epoch) / epoch):
                upper = np.abs(np.ones(len(list_best[0].position)) - list_best[0].position)
                mutation = True
                if random.random() > 0.5:
                    mutated_pos = np.abs(list_best[0].position + random.random() * upper)
                else:
                    mutated_pos = np.abs(list_best[0].position - random.random() * list_best[0].position)
                mutated_pos, graph_mutated= amend_position(mutated_pos)
                fitness_mutated= compute_fitness_vector(mutated_pos)
            pos_new, graph_s = amend_position(pos_new)
            fitness_pos_new = compute_fitness_vector(pos_new)
            w = wolf()
            if mutation and fitness_mutated> fitness_pos_new:
                w.position = mutated_pos
                w.fitness=fitness_mutated
            else:
                w.position = pos_new
                w.fitness = fitness_pos_new

            #augmented_sub_graph = nx.Graph(graph_s)
            #compute_fitness(w, False)
            """
            for pair in w.targets_and_its_covering_sensors:
                for s in pair['sensors']:
                    augmented_sub_graph.add_edge(pair['target']+100000,s)
            sub_metric = metric_closure(augmented_sub_graph, weight='weight')
            remove_nodes = ls.local_search(graph_s, sub_metric, w.covered_targets)
            for r in remove_nodes:
                w.position[r]=0
            w.position=np.array(w.position)
            compute_fitness(w, False)
            """
...
def solve():
    ...
    for current_epoch in range(0, epoch):
        ...
        for idx in range(0, pop_size):
            ...
            print(w.fitness)
            print("terminals before local search are ", list(set(w.covering_sensors)))
            sus_metric = metric_closure(graph_s, weight='weight')
            remove_nodes = ls.local_search(graph_s, sub_metric, list(set(w.covering_sensors)))
            w.position=[0, ] * len(zones)
            for r in remove_nodes:
                w.position[r]=1
            w.position=np.array(w.position)
            compute_fitness(w, False)
            ...
            print(w.fitness)
            graph_s = create_deployment_graph(w.position)
            disjoint_sets = sp.distinct_connected_components(graph_s)
            if len(disjoint_sets) > 1:
                print("##########################################graph  after local search not connected")
                print(disjoint_sets)
            else:
                print("yyyyyyyyyyyyyyyyyyyyyyy graph after local search is connected")
